...
///
/// ## Panics
/// Panics if `T1` and `T2` have different sizes or alignments.
#[must_use]
...
///
/// let mut v = vec![0, 1, 2];
/// let iter: xstd::vec::DrainFilterSwapping<_, _> = v.drain_filter_swapping(|x| *x % 2 == 0);
/// ```
#[derive(Debug)]
...
mod test {
    ...
    fn miri_test_wrong_align() {
        ...
        // the alignment check
        let v: Vec<Gus1> = vec![Gus1::default()];
